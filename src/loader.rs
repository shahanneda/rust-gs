pub mod loader {
    ...
    use crate::scene::scene::{Scene, Splat};
    ...
    pub async fn load_ply() -> Result<(Scene), EvalError> {
        // let body = reqwest::get("http://127.0.0.1:5500/splats/test.txt")
        let body = reqwest::get("http://127.0.0.1:5500/splats/corn.ply")
            .await
            .expect("error")
            .bytes()
            .await
	...
	let scene = Scene{
		splats: vertex_vals.iter().enumerate().map(|(i, splat_ply)| {
