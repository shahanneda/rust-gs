pub mod old_ply_loader {
    ...
    pub async fn load_ply() -> Result<Vec<PlySplat>, EvalError> {
        ...
        console::time_with_label("a");
        let mut reader: Box<dyn Read> = Box::new(body.reader()) as Box<dyn Read>;
        console::time_end_with_label("a");

        console::time_with_label("b");
        let p = Parser::<ply::DefaultElement>::new();
        console::time_end_with_label("b");

        console::time_with_label("c");
        let ply = p.read_ply(&mut reader);
        console::time_end_with_label("c");
